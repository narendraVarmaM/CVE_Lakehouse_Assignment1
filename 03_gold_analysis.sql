...
FROM cse587.silver.core_cves c
LEFT JOIN cse587.silver.affected_products a ON c.cve_id = a.cve_id
...
FROM cse587.silver.affected_products a
INNER JOIN cse587.silver.core_cves c ON a.cve_id = c.cve_id
...
FROM cse587.silver.affected_products ap
FULL OUTER JOIN cse587.silver.core_cves cc ON ap.cve_id = cc.cve_id;
...
FROM cse587.silver.affected_products ap
LEFT ANTI JOIN cse587.silver.core_cves cc ON ap.cve_id = cc.cve_id;
...
SELECT
    ...
    (SELECT COUNT(*) FROM cse587.silver.core_cves WHERE risk_category = 'CRITICAL') as critical_cves,
    (SELECT COUNT(*) FROM cse587.silver.core_cves WHERE risk_category = 'HIGH') as high_risk_cves,
